        ...
        <div className='flex flex-col border-r border-stroke p-2 dark:border-strokedark'>
            <div className="flex flex-col items-center space-y-5">
                ...
                {NAVIGATION.map(({ icon, key, title }) => (
                    <div key={key} className="group space-y-2 flex flex-col text-center hover:cursor-pointer" onClick={() => { handleClick(key) }}>
                        <div className={`mx-auto border rounded-md border-stroke p-2 dark:border-strokedark ${selected === key && "bg-primary bg-opacity-90 text-white"} group-hover:border-primary group-hover:text-primary dark:group-hover:border-primary`}>
                            {icon}
                        </div>
                        <span className={`font-medium text-sm ${selected === key && "text-primary"} group-hover:text-primary`}>
                            {title}
                        </span>
                    </div>
                ))}
